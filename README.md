...
2. **Assortment file** (optional) — supplies the order quantities for the PO import. The register provides the product and supplier data; the assortment provides *how many* to order.

Each file has its own drop box (Register on the left, Assortment on the right), and each box only accepts its own file type. Both can be `.xlsx` or `.csv`, and they can be dropped in either order.

The app reads two assortment shapes automatically: a **multi-tab launch workbook** (one tab per category, each with a `BBB Product ID` and a `Total Units` column — all category tabs are read and combined, summary/utility tabs are skipped), or a simple single sheet with a `SKU` + `Unit QTY` column. The PO quantity comes from `Total Units` (or `Unit QTY`); the join to the register matches on BBB SKU ID, falling back to Vendor Part #.

---

## How it works — three steps

1. **Drop the Master SKU Register** (left box). The app reads it, groups variants into products, maps vendors to their Fulfil names, and shows a summary plus a row-by-row review table with any gaps flagged.
2. **Add the assortment file** (right box, for PO quantities) **and fill any gaps.** A vendor include/exclude panel appears, plus an optional enrichment box and only the manual fields the file is actually missing.
